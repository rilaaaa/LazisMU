...
export default function JournalDetailTable({ journal, searchTerm }: JournalDetailTableProps & { searchTerm: string }) {
  ...
  // Tampilkan semua data jika searchTerm kosong
          ...
          {filteredEntries.map((data, index) => (
            <tr key={index} className="hover:bg-gray-50">
              <td className="px-6 py-4 text-sm text-gray-700">
                {data.tanggal ? new Date(data.tanggal).toLocaleString() : '-'}
              </td>
              ...
              <td className="px-6 py-4 text-sm text-gray-700">{data.nominal || 0}</td>
              <td className="px-6 py-4 text-sm text-gray-700">{data.sumber_dana || 0}</td>
            </tr>
          ))}
        </tbody>
      </table>
